...
//动态调参
void dynamicReconfigureCallback(local_planner::pathFollowerConfig &config, uint32_t level) {
  ...
  ROS_INFO("New maxYawRate: %f", config.maxYawRate);
  ROS_INFO("New maxAccel: %f", config.maxAccel);
  // 根据需要更新其他动态参数
}
...
int main(int argc, char** argv)
{
 ...
 // 插入动态重配置服务器的设置
  ...
  server.setCallback(boost::bind(&dynamicReconfigureCallback, _1, _2));
